...
var CLOUDINARY_CONFIG = {
  cloudName: 'dcywb4jkc',              // Your Cloudinary cloud name
  uploadPreset: 'Gowtham'              // Your unsigned upload preset
};
...
function retryOnUnavailable(operationFn, attemptsLeft, waitMs, maxWaitMs) {
  attemptsLeft = typeof attemptsLeft === 'number' ? attemptsLeft : 6;
  waitMs = typeof waitMs === 'number' ? waitMs : 1200;
  maxWaitMs = typeof maxWaitMs === 'number' ? maxWaitMs : 8000;

  return operationFn().catch(function(error) {
    var code = error && error.code ? error.code : '';
    var retryable = code === 'unavailable' || code === 'deadline-exceeded';

    if (retryable && attemptsLeft > 1) {
      var nextWait = Math.min(waitMs * 2, maxWaitMs);
      return delay(waitMs).then(function() {
        return retryOnUnavailable(operationFn, attemptsLeft - 1, nextWait, maxWaitMs);
      });
    }
    throw error;
  });
...
// ===== PHOTO UPLOAD (Admin) - Uses Cloudinary =====
function handleAdminPhotoUpload(input) {
  ...
  uploadToCloudinary(file, user.uid)
    .then(function(url) {
      // Save URL to Firestore (use set with merge to handle docs that don't exist yet)
    ...
    .catch(function(err) {
      ...
      if (err.message && err.message.indexOf('Cloudinary') !== -1) {
        showToast('📷 Please configure Cloudinary settings in main.js', 'error');
      } else {
...
// ===== PHOTO UPLOAD (Member) - Uses Cloudinary =====
function handleMemberPhotoUpload(input) {
  ...
  uploadToCloudinary(file, user.uid)
    ...
    .then(function(url) {
      ...
      // Also update topbar avatar if exists
...
function uploadToCloudinary(file, userId) {
  return new Promise(function(resolve, reject) {
    ...
    if (CLOUDINARY_CONFIG.cloudName === 'YOUR_CLOUD_NAME' || CLOUDINARY_CONFIG.uploadPreset === 'YOUR_UPLOAD_PRESET') {
      reject(new Error('Cloudinary not configured. Please update CLOUDINARY_CONFIG in main.js'));
      return;
    }
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        // Return the secure URL with transformation for optimized delivery
...
function updateTopbarAvatar(url) {
  ...
  var emoji = document.getElementById('topbarAvatarEmoji');
  ...
  if (topbarAvatar) {
    ...
    // Hide the emoji text if it exists
    var textNode = topbarAvatar.childNodes[0];
    if (textNode && textNode.nodeType === 3) {
      textNode.textContent = '';
    }
    var emojiSpan = topbarAvatar.querySelector('span');
        ...
        .then(function(doc) {
          ...
          if (doc.exists) {
            ...
            if (data.photoURL) {
              ...
              updateTopbarAvatar(data.photoURL);
            }
